...
local awdb_separator = "\171\205\239ipplus360.com"

local geodb_methods = {}
local geodb_mt = {
	__name = "awdblua-database";
...
local function new(contents)
	local start_metadata do
		...
		while true do
			local s, e = contents:find(awdb_separator, start_metadata or init, true)
			if s == nil then break end
			start_metadata = e + 1
		end
		if start_metadata == nil then
			return nil, "Invalid AW Database"
		end
	end
